 ...
 * Validasi token.
 *
 * Coba /account dulu (untuk menampilkan email). TAPI token DigitalOcean model
 * baru bisa dibatasi scope-nya — token yang cuma diberi scope "droplet" akan
 * DITOLAK (401/403) di /account meski sebenarnya valid untuk membuat droplet.
 * Maka kalau /account gagal karena izin, kita uji ke /droplets — endpoint yang
 * memang dipakai alat ini. Kalau itu jalan, token dianggap valid.
 */
async function validateToken(token) {
  try {
    ...
    if (st === 401 || st === 403) {
      // Mungkin token valid tapi tanpa scope "account". Uji lewat droplet.
      try {
        await client(token).get('/droplets', { params: { per_page: 1 } });
        return { ok: true, email: null, status: 'scoped' };
      } catch (e2) {
